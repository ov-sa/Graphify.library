...
---------------------------------------------------------------------------------------------------
zBufferRecovery = { }
function zBufferRecovery.create()
	if zBufferRecovery.shader then return true end
	zBufferRecovery.shader = dxCreateShader( "fx/primitive2D_fixZBuffer.fx" )
	if zBufferRecovery.shader then
		dxSetShaderValue( zBufferRecovery.shader, "viewportSize", guiGetScreenSize() )
		return true
	end
	return false
end
...
function renderZBuffer()
	if zBufferRecovery.shader then
		-- draw the outcome
		dxDrawMaterialPrimitive3D( "trianglelist", zBufferRecovery.shader, false, unpack( trianglelist.plane ) )
	end
end

function destroyZBufferRecovery()
	if zBufferRecovery.shader then
		destroyElement( zBufferRecovery.shader )
		zBufferRecovery.shader = nil
	end
end