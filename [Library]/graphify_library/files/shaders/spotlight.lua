function getSpotLightFX()
    return [[
        ...
        float lightPhi = 0;
        float lightTheta = 0;
        float lightFallOff = 0;
        float3 lightViewDirection = float3(0, 0, -1);
        ...
        bool GetZEnable() {
            if (lightSubdivUnit >= 2) {
                ...
                if (length(sViewInverse[3].xyz - lightPosition) <max(1, tan(lightPhi))*2*lightAttenuation) return false;
                else return true;
            }
            else return true;
        }
        ...
        int GetCullMode() {
            if (lightSubdivUnit >= 2) {
                float4x4 sViewInverse = InverseMatrix(gViewMainScene);
                if ((length(sViewInverse[3].xyz - lightPosition)) <max(1, tan(lightPhi))*lightAttenuation*3.5) return 3;
                else return 2;
            }
            else return 2;
        }
        ...
        PSInput VSHandler_1(VSInput VS) {
            ...
            if (lightSubdivUnit >= 2) {
                VS.Position.z = cos(2*VS.Position.x*PI)*0.5;
                VS.Position.x = sin(2*VS.Position.x*PI)*0.5;
                VS.Position.y += 0.5;
                VS.Position.xz *= VS.Position.y;
                float sphRadius = 1/cos(radians(180/lightSubdivUnit));
                float subDiv = 1 - (1/lightSubdivUnit);
                if (VS.Position.y >= subDiv) {VS.Position.xz = 0; VS.Position.y = subDiv;}
                float __lightAttenuation = lightAttenuation*sphRadius;
                VS.Position.y *= __lightAttenuation;
                VS.Position.xz *= tan(lightPhi)*(2*__lightAttenuation);
            }
            else VS.Position.xy *= lightAttenuation*2.5;	
            VS.TexCoord.x = 1 - VS.TexCoord.x;
            float3 elementRotation = float3((asin(lightViewDirection.z/length(lightViewDirection))), 0, -(atan2(lightViewDirection.x, lightViewDirection.y)));
            float4x4 sWorld = CreateMatrix(lightPosition, elementRotation);
